...
def ConvLayer(input, batch_size, innerLayers, nb_row=2, nb_col=2, subsample=(1,1), border_mode='valid', dim_ordering='default'):
    beforeLayer = ConvReshapeBefore(nb_row, nb_col, subsample=subsample, border_mode=border_mode, dim_ordering=dim_ordering)
    output = beforeLayer(input)
    input_shape = beforeLayer.input_shape
    output_shape = get_conv_output_shape(input_shape[1:], nb_row, nb_col, subsample, dim_ordering, border_mode)
...
class ConvReshapeBefore(Layer):
    ...
    def build(self, input_shape):
        output_shapes = get_conv_output_shape(input_shape[1:], self.nb_row, self.nb_col, self.subsample, self.dim_ordering, self.border_mode)
    ...
    def get_output_shape_for(self, input_shape):
        if self.dim_ordering == 'th':
            return (None, self.nb_features, self.nb_row, self.nb_col)
        elif self.dim_ordering == 'tf':
            return (None, self.nb_row, self.nb_col, self.nb_features)
        else:
